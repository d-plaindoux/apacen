...
subst(((X:E1) -> E2)[X:=_],(X:E1) -> E2,forall(hidden))   :- !.
...
subst(((X:R) * E)[X:=Y],(X:R) * E,exist(hidden))          :- !.
subst(((X1:E1) * E2)[X:=Y],(X1:E3) * E4,exist(L1,L2))     :- !,subst(E1[X:=Y],E3,L1),subst(E2[X:=Y],E4,L2).
subst((E1:E2)[X:=Y],(E3:E4),(L1:L2))                      :- !,subst(E1[X:=Y],E3,L1),subst(E2[X:=Y],E4,L2).
subst((E1 :=: E2)[X:=Y],E3 :=: E4,L1 :=: L2)              :- !,subst(E1[X:=Y],E3,L1),subst(E2[X:=Y],E4,L2).
subst(subst_by(E1,E2),subst_by(E1,E2),subst_by(L1,L2))    :- !,subst(E1[X:=Y],E3,L1),subst(E2[X:=Y],E4,L2).
